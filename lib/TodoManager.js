...
export class TodoManager {
  ...
  async _load() {
    ...
    // Migrate old-shape files IN MEMORY only. We never persist from _load
    // because reads (view/list/show/comments) call it outside a write lock,
    // and _load is also called from inside write locks (the lockfile is not
    // reentrant). The migrated shape is written by the first write operation
    // instead — those run under _withLock and call _save. Read-only usage
    // re-migrates in memory each call (cheap and idempotent).
    return TodoParser.migrate(JSON.parse(content));
  }
